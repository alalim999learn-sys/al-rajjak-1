...
export default function ElectricianChat({ clientData }: { clientData: ClientData }) {
  const [messages, setMessages] = useState<Message[]>([
    { 
      role: 'assistant', 
      content: `Guten Tag! Ich bin আপনার AI-Assistent. ${clientData.welcomeMsg || 'Wie kann ich Ihnen heute helfen?'}` 
    }
  ]);
    ...
    <div style={{
      ...
      {showForm && (
        <div style={{
          ...
          <h4 style={{ fontSize: '16px', fontWeight: 'bold', color: '#0f172a', marginBottom: '5px' }}>Termin vereinbaren</h4>
          <input style={inputStyle} placeholder="Name" />
          <input style={inputStyle} placeholder="Telefonnummer" />
          <textarea style={{ ...inputStyle, height: '80px', resize: 'none' }} placeholder="Was muss repariert werden?"></textarea>
          <button 
            onClick={() => { alert('Gesendet! Wir rufen Sie gleich zurück.'); setShowForm(false); }}
            style={{
        ...
        {messages.map((m, i) => {
          ...
          return (
            <div key={i} style={{
              ...
              boxShadow: m.role === 'assistant' ? '0 2px 5px rgba(0,0,0,0.02)' : 'none'
            }}>
              {cleanContent}
      ...
      <div style={{ padding: '15px', borderTop: '1px solid #f1f5f9', backgroundColor: 'white' }}>
        ...
        {/* Shanon Alam Branding & Safety Note */}
        <div style={{ textAlign: 'center', marginTop: '10px' }}>
          <p style={{ fontSize: '9px', color: '#94a3b8', marginBottom: '2px' }}>
            KI kann Fehler machen. (Zu Ihrer Sicherheit)
          </p>
          <p style={{ 
            fontSize: '9px', color: '#cbd5e1', textTransform: 'uppercase', 
